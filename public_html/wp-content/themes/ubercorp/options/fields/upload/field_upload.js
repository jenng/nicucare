jQuery(document).ready(function(){
	...
	window.send_to_editor = function(html) {
		...
		if (header_clicked) {
			...
			$id =  formfield.split('bgpattern');
			
			jQuery('#'+$id[0]+'bgoptions').fadeIn('slow');
			jQuery('#'+$id[0]+'background').val(imgurl); // Add the new value in hidden element
	...
	jQuery('.nhp-opts-upload-remove').click(function(){
		...
		$id =  $relid.split('bgpattern');
		jQuery('#'+$id[0]+'bgoptions').fadeOut('slow');

		var pattern_value = jQuery('#'+$id[0]+'bgdefault').val();
		
		jQuery('#'+$id[0]+'background').val(pattern_value);
